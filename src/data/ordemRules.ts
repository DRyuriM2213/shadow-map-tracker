...
export interface CharacterSheetData {
  ...
  concept: { origin: string; className: SheetClassName; customClass: string; trail: string; nex: number; rank: string; movement: number; pePerRound: number; freeMode: boolean };
...
export function emptyCharacterSheet(): CharacterSheetData {
  return {
    ...
    concept: { origin: "", className: "Não definida", customClass: "", trail: "", nex: 0, rank: "", movement: 9, pePerRound: 0, freeMode: false },
...
/** Migração não destrutiva: cards, recursos e escolhas antigas continuam intactos. */
export function normalizeSheet(input: unknown): CharacterSheetData {
  ...
  const concept = { ...base.concept, ...(data.concept ?? {}) };
  concept.nex = normalizeNex(Number(concept.nex));
  const attributes = { ...base.attributes, ...(data.attributes ?? {}) };
  let progression = migrateLegacyRitualCards(abilities, normalizeProgression(data.progression));

  const rawProgression = data.progression as Partial<ProgressionState> | undefined;
  const hadCreationMetadata = Boolean(rawProgression?.creation);
  if (!hadCreationMetadata && concept.nex >= 5) {
    const cls = baseClassOf(concept.className);
    progression = {
      ...progression,
      creation: {
        ...
        status: "LEGADA",
        classAtCreation: cls,
        initialInt: Number(attributes.INT ?? 1),
        maxIntGranted: Number(attributes.INT ?? 1),
      },
    };
  }
...
export function creationPointsUsed(attributes: Record<OrdemAttribute, number>) {
  ...
  const withinLimits = zeros <= 1 && values.every((value) => value >= 0 && value <= 3) && spent <= budget;
  return {
    spent,
    budget,
    remaining: Math.max(0, budget - spent),
    valid: withinLimits,
    complete: withinLimits && spent === budget,
  };
}
