...
type MediaActionResponse = { ok?:boolean; error?:string };
...
export function LiveMediaCaster(){
  ...
  useEffect(()=>{
    ...
    if(!stillActive)setTarget("group");
  },[dashboard,target]);
  ...
  return <div className="fixed bottom-4 left-4 z-[90] w-[min(430px,calc(100vw-2rem))]">{open&&<div className="mb-2 max-h-[75vh] overflow-y-auto rounded-sm border border-primary/40 bg-background/98 p-4 shadow-2xl backdrop-blur-xl"><div className="flex items-start gap-3"><div className="flex size-9 items-center justify-center rounded-sm border border-primary/40 bg-primary/10"><Radio className="size-4 text-primary"/></div><div className="min-w-0 flex-1"><p className="stamp text-primary">Broadcast visual</p><h3 className="font-semibold">Mostrar imagem na tela do player</h3><p className="mt-1 text-xs text-muted-foreground">Escolha foto e destino. O terminal recebe automaticamente.</p></div><Button size="sm" variant="ghost" disabled={busy} onClick={()=>setOpen(false)}><X className="size-4"/></Button></div><label className="mt-4 block text-xs text-muted-foreground">Destino<select className="mt-1 h-9 w-full rounded-sm border border-input bg-background px-2 text-sm" value={target} disabled={busy} onChange={e=>setTarget(e.target.value)}><option value="group">Grupo inteiro</option>{(dashboard?.players??[]).filter(p=>p.active).map(p=><option key={p.id} value={p.id}>{p.playerName} · {p.characterName||"sem personagem"}</option>)}</select></label><div className="mt-3 grid gap-2 sm:grid-cols-2"><Input value={title} disabled={busy} onChange={e=>setTitle(e.target.value)} placeholder="Título"/><Input value={caption} disabled={busy} onChange={e=>setCaption(e.target.value)} placeholder="Legenda opcional"/></div><button type="button" disabled={busy} onClick={()=>inputRef.current?.click()} className="mt-3 flex min-h-36 w-full items-center justify-center overflow-hidden rounded-sm border border-dashed border-primary/50 bg-black/20 p-2 disabled:cursor-not-allowed disabled:opacity-60">{imageData?<img src={imageData} alt="Prévia" className="max-h-64 max-w-full object-contain"/>:<span className="flex flex-col items-center gap-2 text-sm text-muted-foreground"><ImageIcon className="size-8"/>Clique para escolher uma foto</span>}</button><input ref={inputRef} hidden type="file" accept="image/*" onChange={onFile}/><div className="mt-3 flex gap-2"><Button className="flex-1" disabled={!imageData||busy} onClick={()=>void send()}><Send className="mr-2 size-4"/>{busy?"Processando…":"EXIBIR AGORA"}</Button>{imageData&&<Button variant="outline" disabled={busy} onClick={()=>setImageData("")}>Limpar</Button>}</div>{error&&<p className="mt-2 rounded-sm border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive">{error}</p>}{notice&&<p className="mt-2 rounded-sm border border-route-verde/40 bg-route-verde/10 p-2 text-xs text-route-verde-claro">{notice}</p>}<div className="mt-5 border-t border-border pt-3"><div className="flex items-center gap-2"><p className="stamp text-muted-foreground">Transmissões recentes</p><Button size="sm" variant="ghost" className="ml-auto" disabled={busy} onClick={()=>void refresh()}><RefreshCw className={`size-3.5 ${busy?"animate-spin":""}`}/></Button><Button size="sm" variant="ghost" className="text-destructive" disabled={busy||!history.some(item=>item.active)} onClick={()=>void clearAll()}><Trash2 className="mr-1 size-3.5"/>Encerrar todas</Button></div><div className="mt-2 space-y-1.5">{history.slice(0,8).map(item=><div key={item.id} className="flex items-center gap-2 rounded-sm border border-border p-2 text-xs"><div className="min-w-0 flex-1"><b className="block truncate">{item.title}</b><span className="text-muted-foreground">{item.targetPlayerId?`${item.targetName??"Player"}${item.characterName?` · ${item.characterName}`:""}`:"Grupo inteiro"} · {item.active?"ATIVA":"encerrada"}</span></div>{item.active&&<Button size="sm" variant="outline" disabled={busy} onClick={()=>void clearOne(item.id)}><Square className="mr-1 size-3"/>Encerrar</Button>}</div>)}{!history.length&&<p className="text-xs text-muted-foreground">Nenhuma imagem transmitida ainda.</p>}</div></div></div>}<Button className="shadow-2xl" variant={open?"secondary":"default"} disabled={busy&&!open} onClick={()=>setOpen(v=>!v)}><Radio className="mr-2 size-4"/>{open?"Fechar broadcast":"Transmitir imagem"}</Button></div>;
}
