...
export function InventoryPanel({ sheet, editable, onChange }: {
  ...
  const visualPercent = Math.min(100, load.percent / 2);
  ...
  return <div className="space-y-4">
    ...
    <section className="rounded-2xl border border-primary/20 bg-primary/[0.035] p-4 sm:p-5">
      ...
      <div className="relative mt-4"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><Input className="h-11 pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar arma, munição ou equipamento..."/></div>
...
function InventoryCard({ item, editable, freeMode, update, remove }: {
  ...
  return <article className="rounded-xl border border-border bg-background/25 p-3.5">
    <div className="flex flex-wrap items-start gap-3">
      <div className="min-w-[180px] flex-1">{isManual || freeMode ? <Input disabled={!editable} className="h-9 font-semibold" value={item.name} onChange={(event) => update({ name: event.target.value })}/> : <div className="flex items-center gap-2"><b>{item.name}</b><span className="rounded-full border border-primary/25 bg-primary/[0.06] px-2 py-0.5 text-[9px] text-primary">CATÁLOGO</span></div>}<p className="mt-1 text-[11px] text-muted-foreground">{catalog ? `${catalog.category} · Cat. ${catalog.rank}` : item.category || "Item manual"}</p></div>
    ...
    {weapon && <div className="mt-3 flex flex-wrap items-center gap-1.5 rounded-xl border border-border/80 bg-black/10 p-2.5"><Swords className="mr-1 size-4 text-primary"/><Chip>Dano {weapon.damage}{weapon.addStrength ? " + FOR" : ""}</Chip><Chip>Crítico {critLabel(weapon.criticalMargin, weapon.criticalMultiplier)}</Chip><Chip>{rangeLabel(weapon.range)}</Chip><Chip>{damageTypeLabel(weapon.damageType)}</Chip>{weapon.ammo && <Chip>Munição: {weapon.ammo}</Chip>}</div>}
