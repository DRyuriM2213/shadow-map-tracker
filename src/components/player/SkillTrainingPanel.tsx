...
import { BookOpen, Brain, Check, ChevronUp, Search, Sparkles, TrendingUp } from "lucide-react";
...
export function SkillTrainingPanel({ sheet, editable, onChange, onRollSkill }: {
  ...
  const pendingPowerSlots = powerSources.reduce((total, choice) => total + remainingPowerTrainingSlots(choice, sheet.progression.skillTrainingGrants), 0);
  const hasNewSkillOpportunity = bonusIntRemaining > 0 || pendingPowerSlots > 0;
  ...
  return <div className="skill-training-v2 space-y-4">
    <section className="skill-training-hero">
      <div className="flex flex-wrap items-start gap-4">
        <div className="min-w-0 flex-1"><p className="stamp text-primary">Perícias & treinamento</p><h2 className="mt-1 font-display text-2xl sm:text-3xl">Máximo atual: {labelTraining(cap)} <span className="font-mono text-primary">+{TRAINING_BONUS[cap]}</span></h2><p className="mt-2 max-w-2xl text-xs leading-relaxed text-muted-foreground">{cap === "TREINADO" ? "Você ainda pode aprender perícias novas, mas o Grau de Treinamento só melhora perícias que já estão em +5. Veterano libera em NEX 35%." : cap === "VETERANO" ? "Veterano (+10) está liberado. Perícias novas continuam vindo de Intelecto ou do poder Treinamento em Perícia; o marco de 35% não cria perícias novas." : "Expert (+15) está liberado. O Grau de Treinamento continua melhorando perícias que você já possui."}</p></div>
      ...
      <div className="skill-rule-grid mt-4">
        <article><BookOpen className="size-4 text-primary"/><div><b>Grau de Treinamento</b><p>NEX 35% e 70%: melhora perícias já treinadas. Não cria uma perícia nova.</p></div></article>
        <article><Brain className="size-4 text-primary"/><div><b>Aumentar Intelecto</b><p>Cada ponto permanente novo de INT libera uma nova perícia Treinada (+5).</p></div></article>
        <article><TrendingUp className="size-4 text-primary"/><div><b>Treinamento em Perícia</b><p>Ao escolher esse poder, você recebe duas escolhas para treinar novas perícias ou elevar graus permitidos pelo NEX.</p></div></article>
      </div>
    </section>

    {hasNewSkillOpportunity && <section className="new-skill-opportunity">
      <div className="flex items-center gap-3"><div className="new-skill-pulse"><Sparkles className="size-4"/></div><div className="min-w-0 flex-1"><p className="stamp text-primary">Nova perícia disponível</p><p className="text-sm font-semibold">{bonusIntRemaining > 0 ? bonusIntRemaining+" por Intelecto" : ""}{bonusIntRemaining > 0 && pendingPowerSlots > 0 ? " · " : ""}{pendingPowerSlots > 0 ? pendingPowerSlots+" pelo poder Treinamento em Perícia" : ""}</p><p className="mt-0.5 text-xs text-muted-foreground">As opções aparecem logo abaixo. Você pode filtrar por “Destreinadas” para encontrar rapidamente uma perícia nova.</p></div><Button type="button" size="sm" variant="outline" onClick={()=>setFilter("DESTREINADO")}>Ver destreinadas</Button></div>
    </section>}
