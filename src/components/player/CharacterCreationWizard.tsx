...
const STEP_GUIDES = [
  "Escolha de onde seu personagem veio. O sistema adiciona automaticamente as perícias e o poder concedidos pela origem.",
  "Escolha como seu personagem atua durante uma investigação. A classe define sua base de combate, perícias e progressão.",
  "Distribua todos os pontos entre os cinco atributos. O contador precisa chegar a zero para continuar.",
  "Complete as escolhas obrigatórias da classe e depois selecione exatamente a quantidade de perícias livres indicada.",
  "Confira o resumo. Nada aqui é uma escolha nova: é só a confirmação antes de entrar em NEX 5%.",
];
const STEP_RULES = [
  "Origem não é profissão atual: ela representa a vida que você levava antes do paranormal entrar no caminho.",
  "Combatente prioriza confronto, Especialista resolve problemas e investiga, Ocultista lida diretamente com o paranormal.",
  "Atributos maiores significam mais dados nos testes ligados a eles. O site impede uma distribuição inválida.",
  "Perícias treinadas recebem o bônus de treinamento. As marcadas como automáticas não gastam suas escolhas livres.",
  "Depois da criação, o próprio site vai avisar quando NEX liberar trilha, atributos, perícias ou poderes.",
];
...
export function CharacterCreationWizard({ sheet, editable, onChange }: { sheet: CharacterSheetData; editable: boolean; onChange: (next: CharacterSheetData) => void }) {
  ...
  const missingMessage = () => {
    if (step === 1) return origin ? "" : "Escolha uma origem para continuar.";
    if (step === 2) return cls ? "" : "Escolha uma classe para continuar.";
    if (step === 3) return points.complete ? "" : "Ainda restam " + points.remaining + " ponto(s) para distribuir.";
    if (step === 4) {
      if (!rule) return "Escolha uma classe antes das perícias.";
      const mandatoryMissing = rule.requiredGroups.filter((group) => !group.some((id) => mandatory.includes(id))).length;
      if (mandatoryMissing > 0) return "Falta completar " + mandatoryMissing + " escolha(s) obrigatória(s) da classe.";
      if (free.length !== allowance) return "Escolha mais " + Math.max(0, allowance - free.length) + " perícia(s) livre(s).";
    }
    return "";
  };
  ...
  return <section className="character-creation-v2 overflow-hidden rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-card/70 to-card/30 shadow-lg shadow-black/20">
    <header className="border-b border-border/70 p-4 sm:p-5"><div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-xl border border-primary/30 bg-primary/10 text-primary"><Shield className="size-5"/></div><div><p className="stamp text-primary">Criação guiada · NEX 0%</p><h2 className="font-display text-2xl">Escolha. O sistema faz as contas.</h2><p className="mt-1 text-xs text-muted-foreground">Não é necessário conhecer Ordem Paranormal para montar a ficha.</p></div></div><div className="mt-4 grid grid-cols-5 gap-1.5">{STEPS.map((label, index) => { const number = index + 1; return <button type="button" key={label} disabled={number > furthest} onClick={() => number <= furthest && setStep(number)} className={`creation-step-pill rounded-lg border px-2 py-2 text-[10px] sm:text-xs ${step === number ? "border-primary bg-primary text-primary-foreground" : step > number ? "border-route-verde/30 bg-route-verde/10 text-route-verde-claro" : "border-border bg-background/30 text-muted-foreground"}`}><span>{number}</span><b>{label}</b></button>; })}</div><div className="creation-progress-track mt-3"><div className="creation-progress-fill" style={{width:((step-1)/4*100)+"%"}}/></div></header>
    <div className="p-4 sm:p-5">
      <div className="creation-guide-grid mb-5">
        <div className="creation-guide-box"><p className="stamp text-primary">O que você precisa fazer</p><p>{STEP_GUIDES[step-1]}</p></div>
        <div className="creation-rule-box"><p className="stamp text-muted-foreground">Regra rápida</p><p>{STEP_RULES[step-1]}</p></div>
      </div>
      ...
      <footer className="mt-5 border-t border-border/70 pt-4"><div className="mb-3 min-h-5 text-xs text-muted-foreground">{step < 5 && !canContinue() ? missingMessage() : step < 5 ? "Etapa pronta. Você pode continuar." : "Revise tudo antes de concluir."}</div><div className="flex justify-between"><Button variant="ghost" disabled={step === 1} onClick={() => { setStep((value) => Math.max(1, value - 1)); setError(""); }}><ChevronLeft className="mr-1 size-4"/>Voltar</Button>{step < 5 && <Button disabled={!canContinue()} onClick={next}>Continuar<ChevronRight className="ml-1 size-4"/></Button>}</div></footer>
    </div>
  </section>;
}
