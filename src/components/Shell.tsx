  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="dossier w-full max-w-md rounded-sm p-6 sm:p-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Universidade Valença — sistema privado de condução de sessão.</p>

        <form className="mt-8 space-y-4" onSubmit={(e) => { e.preventDefault(); submit(); }}>
          <div className="space-y-2">
            <Label htmlFor="pin">PIN do mestre</Label>
            ...
            {!pin && <p className="text-xs text-muted-foreground">Primeiro acesso neste navegador: PIN padrão <b>333</b>.</p>}
          </div>
          {erro && <p className="text-sm text-destructive">{erro}</p>}
          <Button type="submit" className="w-full"><Lock className="mr-2 size-4" />Entrar no painel</Button>
        </form>

        <div className="mt-5 border-t border-border pt-4">
          <button type="button" className="text-xs text-muted-foreground underline" onClick={() => setEditing((v) => !v)}>{editing ? "Cancelar troca de PIN" : "Trocar PIN salvo neste navegador"}</button>
          {editing && <div className="mt-3 flex gap-2"><Input inputMode="numeric" value={newPin} onChange={(e) => setNewPin(e.target.value)} placeholder="Novo PIN (3+ dígitos)" /><Button size="sm" variant="outline" onClick={() => { if (newPin.length < 3) { setErro("O novo PIN precisa ter pelo menos 3 dígitos."); return; } setPin(newPin); setNewPin(""); setEditing(false); setErro("PIN alterado. Use o novo PIN para entrar."); }}>Salvar</Button></div>}
        </div>
        <p className="mt-5 text-xs text-muted-foreground">Acesso local e privado. Não existe visão pública para jogadores nesta versão.</p>
      </div>
    </div>
  );
}
