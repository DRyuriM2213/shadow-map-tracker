...
function TimelinePage() {
  ...
  const activateSafely = (id: string) => {
    const e = TIMELINE.find((x) => x.id === id);
    if (!e) return;
    const future = e.day > session.day || (e.day === session.day && toMinutes(e.time) > toMinutes(session.time));
    if (future && !confirm(`Este evento é futuro (${e.time}). Ativá-lo pode ajustar o relógio. Continuar?`)) return;
