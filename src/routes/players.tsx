...
import { Bell, Cloud, Copy, Eye, FileWarning, Goal, KeyRound, Map as MapIcon, Radio, RefreshCw, Send, Shield, Sparkles, Trash2, UserPlus, Users } from "lucide-react";

export const Route = createFileRoute("/players")({ component: PlayersPage });

type Role = "AGENTE_DA_ORDEM" | "VILAO" | "CIVIL";
type SpecialEventKind = "EVENTO_VISAO" | "EVENTO_SEGREDO" | "EVENTO_OBJETIVO" | "EVENTO_INTERFERENCIA";
const SPECIAL_EVENT_META: Record<SpecialEventKind,{label:string;title:string;body:string}> = {
  EVENTO_VISAO:{label:"Visão Paranormal",title:"Você vê algo que não deveria existir",body:"Por um instante, a realidade perde a forma. Há uma imagem presa atrás dos seus olhos — e ela parece estar olhando de volta."},
  EVENTO_SEGREDO:{label:"Mensagem Secreta",title:"Canal restrito aberto",body:"Uma informação chegou apenas ao seu terminal. Você decide quando — e se — vai compartilhá-la."},
  EVENTO_OBJETIVO:{label:"Novo Objetivo",title:"Prioridade alterada",body:"Uma nova diretriz foi marcada como urgente. Reavalie o que você sabe antes de agir."},
  EVENTO_INTERFERENCIA:{label:"Interferência",title:"A Membrana está instável",body:"O terminal detectou um padrão impossível no sinal. Alguma coisa está perto demais."},
};
...
function PlayersPage(){
  ...
  const [specialKind,setSpecialKind]=useState<SpecialEventKind>("EVENTO_VISAO");
  const [specialTitle,setSpecialTitle]=useState(SPECIAL_EVENT_META.EVENTO_VISAO.title);
  const [specialBody,setSpecialBody]=useState(SPECIAL_EVENT_META.EVENTO_VISAO.body);
  ...
  const releaseTranscendence=async(p:PlayerSummary)=>{const t=requireMasterToken();if(!t)return;if(!confirm("Liberar o chamado do Outro Lado para "+p.playerName+"? Isso envia apenas o convite narrativo; nenhum poder será concedido automaticamente."))return;try{const r=await rpc<{ok?:boolean;error?:string}>("master_send_notification",{p_token:t,p_targets:[p.id],p_kind:"TRANSCENDENCIA",p_title:"O Outro Lado está chamando",p_body:"Há algo além da Membrana tocando a sua presença. Você pode responder ao chamado quando estiver pronto."});if(r?.ok===false)throw new Error(r.error||"Falha ao liberar transcendência");}catch(e){setError(e instanceof Error?e.message:"Falha ao liberar transcendência");}};
  ...
  const chooseSpecialKind=(kind:SpecialEventKind)=>{const meta=SPECIAL_EVENT_META[kind];setSpecialKind(kind);setSpecialTitle(meta.title);setSpecialBody(meta.body);};
  const sendSpecialEvent=async()=>{const t=requireMasterToken();if(!t||!specialTitle.trim())return;if(!validateTargets()){setError("Selecione pelo menos um player");return;}try{const r=await rpc<{ok?:boolean;error?:string}>("master_send_notification",{p_token:t,p_targets:targetIds,p_kind:specialKind,p_title:specialTitle,p_body:specialBody});if(r?.ok===false)throw new Error(r.error||"Falha ao enviar evento especial");}catch(e){setError(e instanceof Error?e.message:"Falha ao enviar evento especial");}};
  ...
  return <Shell><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="dossier p-5"><div className="flex flex-wrap items-start gap-3"><div><p className="stamp text-primary">Eventos cinematográficos</p><h2 className="text-xl font-semibold">Interferir no terminal do player</h2><p className="mt-1 max-w-2xl text-xs text-muted-foreground">Usa o mesmo destino escolhido acima. O evento aparece fullscreen, com som e animação, sem alterar nenhuma regra da ficha.</p></div><Radio className="ml-auto size-6 text-primary"/></div>
      <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{(Object.keys(SPECIAL_EVENT_META) as SpecialEventKind[]).map(kind=>{const meta=SPECIAL_EVENT_META[kind];const Icon=kind==="EVENTO_VISAO"?Eye:kind==="EVENTO_SEGREDO"?FileWarning:kind==="EVENTO_OBJETIVO"?Goal:Radio;return <button type="button" key={kind} data-active={specialKind===kind} className="master-special-event-choice" onClick={()=>chooseSpecialKind(kind)}><Icon className="size-4"/><span>{meta.label}</span></button>})}</div>
      <div className="mt-3 grid gap-2 lg:grid-cols-[.8fr_1.2fr]"><Input value={specialTitle} onChange={e=>setSpecialTitle(e.target.value)} placeholder="Título do evento"/><Textarea className="min-h-24" value={specialBody} onChange={e=>setSpecialBody(e.target.value)} placeholder="Mensagem narrativa"/></div>
      <Button className="mt-3" disabled={!specialTitle.trim()} onClick={()=>void sendSpecialEvent()}><Radio className="mr-1 size-4"/>Disparar evento especial</Button>
    </section>
...
function PlayerCard({p,onEdit,onDetail,onPreview,onPin,onTranscend,onDelete}:{p:PlayerSummary;onEdit:()=>void;onDetail:()=>void;onPreview:()=>void;onPin:()=>void;onTranscend:()=>void;onDelete:()=>void}){const online=p.lastSeen?Date.now()-new Date(p.lastSeen).getTime()<120000:false;return <article className="dossier p-4"><div className="flex gap-3"><div className={`flex size-11 items-center justify-center rounded-xl border ${p.roleType==="AGENTE_DA_ORDEM"?"border-cyan-500/60 bg-cyan-950/50":p.roleType==="VILAO"?"border-red-600/60 bg-red-950/50":"border-amber-500/60 bg-amber-950/30"}`}><Shield className="size-5"/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><b>{p.playerName}</b><span title={online?"online recentemente":"offline"} className={`size-2 rounded-full ${online?"bg-green-400":"bg-muted-foreground"}`}/></div><p className="text-sm text-muted-foreground">{p.characterName||"Ficha sem nome"}</p><p className="stamp mt-1 text-[9px]">{p.roleType.replaceAll("_"," ")} · {p.active?"ativo":"inativo"}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={onDetail}>Ficha / dados</Button><Button size="sm" variant="outline" onClick={onPreview}><Eye className="mr-1 size-3.5"/>Ver como player</Button><Button size="sm" variant="ghost" onClick={onEdit}>Editar</Button><Button size="sm" variant="ghost" onClick={onPin}><KeyRound className="mr-1 size-3.5"/>Novo PIN</Button></div><Button size="sm" variant="outline" className="mt-2 w-full border-primary/30 bg-primary/[.045] text-primary hover:bg-primary/10" onClick={onTranscend}><Sparkles className="mr-1 size-3.5"/>Liberar Transcendência</Button><Button size="sm" variant="ghost" className="mt-2 w-full text-destructive" onClick={onDelete}><Trash2 className="mr-1 size-3.5"/>Excluir</Button></article>}
