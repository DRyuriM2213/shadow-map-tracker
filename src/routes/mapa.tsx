...
export const Route = createFileRoute("/mapa")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Plantas da Universidade Valença com áreas clicáveis ligadas às salas da campanha.",
      },
        ...
        <header className="flex flex-wrap items-end gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Clique em uma área da planta para abrir a sala. Áreas sem posição aparecem na lista ao lado.
            </p>
          </div>
          <div className="ml-auto flex flex-wrap gap-2 text-xs">
            ...
            <Button size="sm" variant={numerado ? "default" : "outline"} onClick={() => setNumerado((v) => !v)}>
              {numerado ? "Versão numerada" : "Versão limpa"}
            </Button>
            <Button size="sm" variant={editar ? "destructive" : "ghost"} onClick={() => setEditar((v) => !v)}>
              {editar ? "Sair da edição" : "Editar áreas"}
            </Button>
          </div>
        </header>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          <div className="dossier relative overflow-hidden rounded-sm p-2">
                  ...
                  <button
                    ...
                    className={`absolute rounded-sm border-2 transition-colors ${
                      ativo
                        ...
                        : h.restricted
                          ? "border-destructive/60 bg-destructive/10 hover:bg-destructive/25"
                          : "border-transparent bg-primary/0 hover:border-primary hover:bg-primary/20"
                    }`}
                  >
            ...
            <p className="mt-2 text-center text-[11px] text-muted-foreground">
              Coloque as imagens em <code>public/mapa/</code> com os nomes definidos em <code>src/data/map.ts</code>.
            </p>
          </div>
                ...
                <div key={h.id} className="rounded-sm border border-border p-2">
                  <button className="w-full text-left" onClick={() => setAberto(h)}>
                    <p className="text-sm font-semibold">
                      {h.name}
                      {h.restricted && <span className="stamp ml-2 text-destructive">restrito</span>}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      {loc ? `${cluesForLocation(loc.id).length} achados` : "sem sala vinculada"}
                      {p ? "" : " · sem posição no mapa"}
                    </p>
                  </button>
                  {editar && (
              ...
              {aberto.locationId ? (
                ...
                <div className="space-y-3 text-sm">
                  <p className="text-muted-foreground">
                    Esta área ainda não está vinculada a uma sala da campanha. Você pode registrar a movimentação do
                    grupo mesmo assim.
                  </p>
                  <Button
                    onClick={() => {
                      logAction("local", `Grupo foi para: ${aberto.name}`);
                      setAberto(null);
                    }}
                  >
