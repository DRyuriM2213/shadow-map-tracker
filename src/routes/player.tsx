...
function PlayerPage() {
  ...
  const openCombat=()=>{playSound("click");setCombatOpen(true);};
...
function Documents({documents}:{documents:CloudDocument[]}){return <section className="investigation-section"><div className="investigation-section-title"><div><p className="stamp text-primary">Arquivo liberado</p><h2 className="font-display text-3xl">Documentos</h2><p>Relatórios, registros e arquivos que chegaram ao seu dossiê.</p></div><BookOpen className="size-6 text-primary"/></div><div className="document-grid mt-4">{documents.map((d,index)=><article key={d.id} className="case-document"><div className="document-fold" aria-hidden="true"/><div className="flex items-center gap-2"><span className="document-index">{String(index+1).padStart(2,"0")}</span><FileText className="size-4 text-primary"/><span className="stamp text-[9px] text-muted-foreground">arquivo autorizado</span></div><h3>{d.title}</h3><p>{d.description}</p>{(d.privateMessage??d.private_message)&&<div className="document-note">{d.privateMessage??d.private_message}</div>}</article>)}{documents.length===0&&<Empty text="Nenhum documento foi liberado para você ainda."/>}</div></section>}
