...
function Resource({label,current,max}:{label:string;current:number;max:number}){const pct=max>0?Math.max(0,Math.min(100,current/max*100)):0;return <div className="player-resource" data-resource={label.toLowerCase()} data-low={max > 0 && pct <= 25 ? "true" : "false"}><div className="flex items-end"><span className="stamp text-muted-foreground">{label}</span><b className="ml-auto font-mono text-xl">{current}<span className="text-xs font-normal text-muted-foreground">/{max}</span></b></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-secondary/80"><div className="player-resource-fill h-full rounded-full bg-primary" style={{width:pct+"%"}}/></div></div>}
